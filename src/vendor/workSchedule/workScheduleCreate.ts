...
export async function _workScheduleCreate(opts: _CreateArgs) {
  const { workschedule } = opts || {};
  const res = await client.post(`/workschedules`, workschedule);
  return res?.data?.data[0];
}

export function workScheduleCreate(opts: CreateArgs) {
  ...
  return _workScheduleCreate({ ...opts, projection });
}
